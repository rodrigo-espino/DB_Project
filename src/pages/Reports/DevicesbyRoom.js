...
export function DevicesbyRoom() {
  const [data, setData] = useState([]);

  const getData= async () => {
  const res = await fetch(`${API}/consults/devicesbyroom`)
...
<Page size="A4" style={styles.page} title="Class by Instructor">
  <View style={styles.section}>
    <Text style={styles.title}>Devices by Rooms</Text>
    <Table
            data={data}
        ...
        >
            ...
            <TableBody>
                ...
                <DataTableCell getContent={(r) => r.location}  style={styles.tableBody}/>
                <DataTableCell getContent={(r) => r.typeofR}  style={styles.tableBody}/>
